...
export default function WalletPage() {
  ...
  const handleDeposit = async () => {
    ...
    setProcessing(true);
    try {
      await apiClient.post('/wallets/deposit', { amount: parseFloat(amount) });
      await fetchWallet();
      setAmount('');
      setShowDeposit(false);
    } catch (error) {
      console.error('Deposit failed, using mock:', error);
  ...
  const handleWithdraw = async () => {
    ...
    setProcessing(true);
    try {
      await apiClient.post('/wallets/withdraw', { amount: parseFloat(amount) });
      await fetchWallet();
      setAmount('');
      setShowWithdraw(false);
    } catch (error) {
      console.error('Withdrawal failed, using mock:', error);
      // Mock withdraw for testing
