...
export default function StaffApplicationReviewPage() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
      // Backend uses PUT /applications/{id} with status in body, not /applications/{id}/status
      await apiClient.put(`/applications/${applicationId}`, { status: newStatus });
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <button
          onClick={() => router.push(`/staff/${params.staff_type}/${params.staff_id}`)}
          style={{
          ...
          <div>
            <div style={{
              ...
              <div style={{
                ...
                {[
                  { label: 'Requested Amount', value: `$${application.requestedAmount?.toLocaleString() || '0'}` },
                  { label: 'Loan Term', value: `${application.requestedTerm || application.requestedTermMonths || 0} months` },
                  { label: 'Purpose', value: application.purpose || 'Personal' },
                  { label: 'Monthly Income', value: `$${application.monthlyIncome?.toLocaleString() || 'N/A'}` },
                ].map((item, idx) => (
                  <div key={idx} style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: premiumTheme.spacing.md }}>
                {[
                  { label: 'Verify Documents', path: `/staff/${params.staff_type}/${params.staff_id}/applications/${params.application_id}/verification` },
                  { label: 'Risk Assessment', path: `/staff/${params.staff_type}/${params.staff_id}/applications/${params.application_id}/risk` },
                  { label: 'Generate Offer', path: `/staff/${params.staff_type}/${params.staff_id}/applications/${params.application_id}/offer/new` },
                  { label: 'View Contract', path: `/staff/${params.staff_type}/${params.staff_id}/applications/${params.application_id}/contract` },
                ].map((action, idx) => (
                  <button
                    key={idx}
