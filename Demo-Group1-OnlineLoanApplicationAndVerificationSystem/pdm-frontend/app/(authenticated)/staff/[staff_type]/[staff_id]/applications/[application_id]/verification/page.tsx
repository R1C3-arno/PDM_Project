...
export default function VerificationConsolePage() {
  ...
  const loadData = async () => {
    try {
      const docsData = await apiClient.get(`/documents/application/${params.application_id}`);
      setDocuments(Array.isArray(docsData) ? docsData : []);

      try {
        const verData = await apiClient.get(`/verification/application/${params.application_id}`) as any;
