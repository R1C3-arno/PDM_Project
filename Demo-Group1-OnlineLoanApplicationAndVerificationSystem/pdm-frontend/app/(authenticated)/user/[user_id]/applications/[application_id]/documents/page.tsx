...
export default function DocumentUploadPage() {
  ...
  const loadDocuments = async () => {
    try {
      const data = await apiClient.get(`/documents/application/${applicationId}`);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const payload = {
        ...
        fileName: file.name,
        fileUrl: `/uploads/${applicationId}/${selectedDocType}_${Date.now()}_${file.name}`,
        fileSize: file.size,
      };
              ...
              <div key={doc.type} style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    opacity: uploadingType === doc.type ? 0.7 : 1,
                  }}
                >
                  {uploadingType === doc.type ? 'Uploading...' : uploaded ? 'Replace Document' : 'Upload Document'}
                </button>
              </div>
            );
          })}
