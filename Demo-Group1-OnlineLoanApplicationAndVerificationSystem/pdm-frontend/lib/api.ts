...
class ApiClient {
  ...
  // Auth response type matching backend AuthResponse DTO
  // Note: JWT token is set as HttpOnly cookie by backend, not returned in response body

  async login(email: string, password: string): Promise<{ user: import('@/types').User; message: string; success: boolean }> {
    try {
      ...
      return result as { user: import('@/types').User; message: string; success: boolean };
    } catch (error) {
  ...
  async register(data: { email: string; password: string; fullName: string; phone?: string }): Promise<{ user: import('@/types').User; message: string; success: boolean }> {
    try {
